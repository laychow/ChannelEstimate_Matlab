...
function MSE=Estimate_U_test1(L,sigma_p)
d=1;    %天线间隔
lambda=2;  %载波波长 
M=128; %天线数
P=100; %路径数
...
if tau~=32                               %SBEM方法
    ...
    if Group_number>tau
        ...
        for r=1:Reuse
            ...
            for g=(1+(r-1)*tau):(tau+(r-1)*tau)
                ...
                Y(:,:,r)=Y(:,:,r)+Y_temp*(S(:,(g-(r-1)*tau))')+sqrt(sigma_n/2)*(randn(M,L) + 1i*randn(M,L));
                count=count+1;
  ...
  %          Y(:,:,r)=Y(:,:,r)+sqrt(sigma_n/2)*(randn(M,L) + 1i*randn(M,L));
        ...
        for g=1:Group_number
            ...
            Y=Y+Y_temp*(S(1:L,g)')+sqrt(sigma_n/2)*(randn(M,L) + 1i*randn(M,L));%%%%%可能分组之后的总组数比tau大，就会报错
        end
