...
for g=1:K
    ...
    while k+a<=K
        if B_index_grouped(2,k+a)~=g
                ...
                if (B_index_grouped(1,k+a)-B_index_grouped(1,k))>=(tau_final+OMEGA-1)&&(B_index_grouped(1,k_s)+M-B_index_grouped(1,k+a))>=(tau_final+OMEGA-1)&&(B_index_grouped(2,k+a)==0)
...
if Group_number>tau_final
    Reuse=ceil(Group_number/tau_final);
    Y=zeros(M,L,Reuse);
    count=1;
    for r=1:Reuse

        for g=(1+(r-1)*tau_final):(tau_final+(r-1)*tau_final)
            Y_temp=zeros(M,1);
            for k=1:K
            ...
            Y(:,:,r)=Y(:,:,r)+Y_temp*(S(:,(g-(r-1)*tau_final))');
            count=count+1;
    ...
    for r=1:Reuse
        ...
        for g=(1+(r-1)*tau_final):(tau_final+(r-1)*tau_final)
          y_g(:,:,g)=(1/(L*sigma_p))*Y(:,:,r)*S(:,(g-(r-1)*tau_final));
          count=count+1;
          if count>Group_number
              break;
          end
        ...
        for k=1:K
            if B_index_grouped(2,k)==g
               ...
               %  h_es_ul_tempp(index_t:index_t+tau-1,:,k)=h_es_ul_temp(index_t:index_t+tau-1,:,k);
                 if(index_t<(M-tau_final+1))
                     h_es_ul_tempp(index_t:index_t+tau_final-1,:,k)=h_es_ul_temp(index_t:index_t+tau_final-1,:,k);
                 else
                     h_es_ul_tempp(index_t:M,:,k)=h_es_ul_temp(index_t:M,:,k);
                     h_es_ul_tempp(1:index_t+tau_final-M,:,k)=h_es_ul_temp(1:index_t+tau_final-M,:,k);
                 end
